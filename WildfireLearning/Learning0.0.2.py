...
for motion in data['Motions']:
    for i in range(len(motion['Frames']) - num_frames + 1):
        sequence = [info['FrameInfo'] for info in motion['Frames'][i:i+num_frames]]
...
# Define LSTM model
model = Sequential([
    LSTM(256, return_sequences=True, kernel_regularizer=l2(0.001), input_shape=(num_frames, X_train.shape[2])),
    BatchNormalization(),
    Dropout(0.5),
